...
### Ozroq oyin oynab ol, bu senga yoqadi!
<!DOCTYPE html>
<html lang="uz">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>So‘z Top O‘yini</title>
  <style>
    body {
...
<body>
  <h1>🎮 So‘z Top O‘yini</h1>
  <div id="wordDisplay"></div>
  <input type="text" id="letterInput" maxlength="1" placeholder="Harf">
  <button onclick="guessLetter()">Top!</button>
  <div id="remaining"></div>
  <div id="message"></div>

  <script>
    const words = ["dasturchi", "innovatsiya", "algoritm", "texnologiya", "informatika", "rivojlanish", "kompyuter", "programma", "maʼlumotlar", "sunʼiyintellekt"];
    ...
    function guessLetter() {
      ...
      if (!letter.match(/[a-zа-яёўқғҳ]/i)) return;
    ...
    function updateStatus() {
      document.getElementById("remaining").textContent = `Qolgan imkoniyatlar: ${remainingAttempts}`;
      
      if (remainingAttempts <= 0) {
        document.getElementById("message").textContent = `😢 Yutqazdingiz! So‘z: ${chosenWord}`;
        document.getElementById("letterInput").disabled = true;
      }

      if (chosenWord.split("").every(letter => guessedLetters.includes(letter))) {
        document.getElementById("message").textContent = "🎉 Tabriklayman! So‘zni topdingiz!";
        document.getElementById("letterInput").disabled = true;
      }
    }
